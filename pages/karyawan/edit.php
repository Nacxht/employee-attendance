...
$data_karyawan = $db->query($select_query)->fetch_assoc();
...
if (isset($_POST["register"])) {
    ...
    NIP = ?, username = ?, email = ?, nama = ?, umur = ?, jenis_kelamin = ?,
    ...
    if ($result) {
        echo "<script>alert('Berhasil mengedit data karyawan!');</script>";
        header("Location: daftar.php");

        exit;
    } else {
        echo "<script>alert('Gagal mengedet data karyawan!');</script>";
    ...
    <div class="flex h-screen">
        <div class="w-64 bg-red-800 text-white p-4 hidden md:block">
            ...
            <nav class="space-y-1">
                ...
                <a href="../absensi.php" class="flex items-center space-x-2 p-3 rounded-lg hover:bg-red-700">
                ...
                <div class="flex justify-center">
                    <form action="edit.php" method="post" class="grid grid-cols-2 w-fit gap-[0.5rem]">
                        <div class="nip">
                            ...
                            <input type="number" name="nip" id="nip" value="<?= $data_karyawan["NIP"] ?>" class="w-[20rem] p-[0.3rem] rounded-md border border-gray-500" required disabled>
                        </div>

                        <div class="username">
                            <label for="username" class="block capitalize">Username</label>
                            <input type="text" name="username" id="username" value="<?= $data_karyawan["username"] ?>" class="w-[20rem] p-[0.3rem] rounded-md border border-gray-500" required>
                        </div>
                        ...
                        <div class="button">
                            <button type="submit" name="register" value="register" class="bg-green-700 px-3 py-1 rounded-md font-bold text-white hover:bg-green-800 transition-all">
                                Submit
                            </button>
                        </div>
                    </form>
